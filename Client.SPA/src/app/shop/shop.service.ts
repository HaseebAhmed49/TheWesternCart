...
/*
  getClothingItems(useCache = true): Observable<IPagination<ClothingItem[]>> {
    if (!useCache) this.clothingCache = new Map();
    if (useCache && this.clothingCache.size > 0) {
      const key = Object.values(this.clothingParams).join('-');
      if (this.clothingCache.has(key)) {
        this.pagination = this.clothingCache.get(key);
        return of(this.pagination!);
      }
    }
    let params = this.getHttpParams(this.clothingParams);
    return this.http.get<IPagination<ClothingItem[]>>(this.baseUrl + 'clothing', { params }).pipe(
      map(response => {
        this.clothingCache.set(Object.values(this.clothingParams).join('-'), response);
        this.pagination = response;
        return response;
      })
    );
  }
  private getHttpParams(clothingParams: ClothingParams) {
    let params = new HttpParams();
    if (clothingParams.clothingBrandId) {
      params = params.append('clothingBrandId', clothingParams.clothingBrandId);
    }
    if (clothingParams.gender) {
      params = params.append('gender', clothingParams.gender);
    }
    if (clothingParams.size) {
      params = params.append('size', clothingParams.size);
    }
    if (clothingParams.category) {
      params = params.append('category', clothingParams.category);
    }
    if (clothingParams.sort) {
      params = params.append('sort', clothingParams.sort);
    }
    if (clothingParams.search) {
      params = params.append('search', clothingParams.search);
    }
    params = params.append('pageIndex', clothingParams.pageIndex.toString());
    params = params.append('pageSize', clothingParams.pageSize.toString());
    return params;
  }
  */
}